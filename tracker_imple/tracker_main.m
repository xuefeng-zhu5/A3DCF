%% Demo Robust Visual Object Tracking via Adaptive Attribute-Aware
...
mask_search_window = cellfun(@(sz,feature) ones(round(currentScaleFactor(feature.fparams.feature_is_deep+1)*sz)) * 0, img_support_sz', features, 'uniformoutput', false);
...
% >
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set cropping matrix P by hand (post processing mask P)
if sum(feature_info.feature_is_deep) > 0
    index = find(feature_info.feature_is_deep==1,1);
    crop_mask_sz = feature_info.data_sz_block{index};
    crop_mask = ones(crop_mask_sz) * 0;
    deep_cell_sz = feature_info.min_cell_size(index);
    target_sz_deep = round(base_target_sz(2,:)/deep_cell_sz * 1.3);  
    target_sz_deep(target_sz_deep>crop_mask_sz) = crop_mask_sz(1);
    target_mask_deep = ones(target_sz_deep)*1.2;
    target_mask_deep_range = zeros(2, 2);
    for i = 1:2
        target_mask_deep_range(i,:) = [0,  target_sz_deep(i) - 1] - floor(target_sz_deep(i) / 2);
    end
    crop_mask_center = floor((crop_mask_sz + 1)/ 2) + mod(crop_mask_sz + 1,2);
    target_deep_h = (crop_mask_center(1) + target_mask_deep_range(1,1)) : (crop_mask_center(1) + target_mask_deep_range(1,2));
    target_deep_w = (crop_mask_center(2) + target_mask_deep_range(2,1)) : (crop_mask_center(2) + target_mask_deep_range(2,2));
    crop_mask(target_deep_h, target_deep_w) = target_mask_deep;
%     D = strel('disk', 2);
%     crop_mask = imdilate(crop_mask, D);
    params.priori_mask = crop_mask;
end
    ...
    if params.vis_res
        ...
        if seq.frame == 1  %first frame, create GUI
            ...
            axis off;axis image;set(gca, 'Units', 'normalized', 'Position', [0 0 1 1])
           	...
           	resp_sz = round(img_sample_sz(1,:)*currentScaleFactor(1,:)*scaleFactors(sind));
